...
import com.vaadin.battle.station.backend.ValidPasswordField;
...
public class EditPersonalInfo extends VerticalLayout {
    ...
    public EditPersonalInfo() {
        ...
        currUserDetailsForm.setVisible(true);
    ...
    private FormLayout createCurUserUI() {
        ...
        setCurrentValues(username,password,access);
        configure(access,password,username);
    ...
    private void configure(TextField access,PasswordField password,TextField username) {
        access.setEnabled(false);
    }
    private void setCurrentValues(TextField username, PasswordField password, TextField access) {
        int batchNo = 0;
